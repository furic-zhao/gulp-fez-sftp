...
module.exports = function(options) {
    ...
    if (options.host === undefined) {
        throw new PluginError('gulp-fez-sftp', '`host` required.');
    }
    ...
    if (options.authKey && fs.existsSync(authFile)) {
        ...
        if (!auth)
            this.emit('error', new PluginError('gulp-fez-sftp', 'Could not find authkey in .ftppass'));
    ...
    if (key) {
        ...
        if (key.location) {
            ...
            this.emit('error', new PluginError('gulp-fez-sftp', 'Cannot find RSA key, searched: ' + key.location.join(', ')));
        }
    ...
    var pool = function(remotePath, uploader) { // method to get cache or create connection
        ...
        if (options.password) {
            fancyLog('Authenticating with password.');
        } else if (key) {
            fancyLog('Authenticating with private key.');
        }
        ...
        c.on('ready', function() {
            ...
            c.sftp(function(err, sftp) {
                ...
                sftp.on('end', function() {
                    fancyLog('SFTP :: SFTP session closed');
                    sftpCache = null;
                    if (!finished)
                        this.emit('error', new PluginError('gulp-fez-sftp', "SFTP abrupt closure"));
                });
        ...
        c.on('error', function(err) {
            self.emit('error', new PluginError('gulp-fez-sftp', err));
            //return cb(err);
        });
        c.on('end', function() {
            fancyLog('Connection :: end');
        });
        c.on('close', function(err) {
            if (!finished) {
                fancyLog('gulp-fez-sftp', "SFTP abrupt closure");
                self.emit('error', new PluginError('gulp-fez-sftp', "SFTP abrupt closure"));
            }
            if (err) {
                fancyLog('Connection :: close, ', colors.red('Error: ' + err));
            } else {
                fancyLog('Connection :: closed');
            }
    ...
    return through.obj(function(file, enc, cb) {
        ...
        pool.call(this, finalRemotePath, function(sftp) {
            ...
            async.whilst(function() {
                ...
                sftp.exists(d, function(exist) {
                    if (!exist) {
                        sftp.mkdir(d, { mode: '0755' }, function(err) { //REMOTE PATH
                            if (err) {
                                fancyLog('SFTP Mkdir Error:', colors.red(err + " " + d));
                            } else {
                                fancyLog('SFTP Created:', colors.green(d));
                            }
                            next();
                        });
                    } else {
                ...
                stream.on('drain', function() {
                    ...
                    p = Math.min(100, p);
                    fancyLog('gulp-fez-sftp:', finalRemotePath, "uploaded", (uploadedBytes / 1000) + "kb");
                });
                ...
                stream.on('close', function(err) {
                    ...
                    if (err)
                        this.emit('error', new PluginError('gulp-fez-sftp', err));
                    else {
                        if (logFiles) {
                            fancyLog('gulp-fez-sftp:', colors.green('Uploaded: ') +
                                file.relative +
                                colors.green(' => ') +
                                finalRemotePath);
                        }
        ...
        if (fileCount > 0) {
            fancyLog('gulp-fez-sftp:', colors.green(fileCount, fileCount === 1 ? 'file' : 'files', 'uploaded successfully'));
        } else {
            fancyLog('gulp-fez-sftp:', colors.yellow('No files uploaded'));
        }
        finished = true;
        if (sftpCache)
            sftpCache.end();
